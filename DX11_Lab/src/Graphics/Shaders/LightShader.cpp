...
bool LightShader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (BaseShader::CompileShaderFromFile(device, hwnd, vsFilename,
		"LightVertexShader", "vs_5_0", &vertexShaderBuffer) == false)
		return false;

	if (BaseShader::CompileShaderFromFile(device, hwnd, psFilename,
		"LightPixelShader", "ps_5_0", &pixelShaderBuffer) == false)
		return false;
	...
	const char* names[] = { "POSITION", "TEXCOORD", "NORMAL" };
	DXGI_FORMAT formats[] = { DXGI_FORMAT_R32G32B32_FLOAT,
